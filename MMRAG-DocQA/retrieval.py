...
import faiss
from mmrag_config import get_openai_client, get_model_name
import os
import numpy as np
...
def get_step_back_query(query: str) -> str:
    ...
    completion = llm.chat.completions.create(model=get_model_name("llm"),
                temperature=0,
                messages=[
...
class VectorRetriever:
    ...
    def _set_up_llm(self):
        return get_openai_client("embedding")
    ...
    def get_strings_cosine_similarity(str1, str2):
        llm = get_openai_client("embedding")
        embeddings = llm.embeddings.create(input=[str1, str2], model=get_model_name("embedding"))
    ...
    def retrieve_by_document_name(self, document_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        target_report = None
